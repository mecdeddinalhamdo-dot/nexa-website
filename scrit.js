...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // نسخة موحدة تمنع تعارض أزرار الثيم وتحفظ الاختيار
    ...
    function setTheme(theme) {
        ...
        htmlEl.classList.toggle('dark', safeTheme === 'dark');
        localStorage.setItem('nexa_theme', safeTheme);

        if (themeToggle) {
            themeToggle.innerHTML = safeTheme === 'dark'
                ? '<i class="fa-solid fa-sun"></i>'
                : '<i class="fa-solid fa-moon"></i>';
            themeToggle.setAttribute('aria-label', safeTheme === 'dark' ? 'تفعيل الوضع النهاري' : 'تفعيل الوضع الليلي');
        }
    }

    // الوضع النهاري هو الافتراضي، مع احترام الاختيار المحفوظ
...
(() => {
  ...
  // theme persistence is handled by the unified theme system above.
  // Do not attach another click listener here because duplicate listeners
  // can cause the theme to switch twice and appear to stop working.
})();
